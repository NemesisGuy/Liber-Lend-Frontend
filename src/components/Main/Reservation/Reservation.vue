...
export default {
  ...
  methods: {
    ...
    fetchBooksList() {
      this.loadingModal.show = true;
      axios
          .get("http://localhost:8080/api/admin/books/all") // Update the endpoint to fetch books
          .then((response) => {
          ...
          .catch((error) => {
            console.log(error);
            this.failModal.message = "Failed to fetch books list"; // Update the error message
            this.failModal.show = true;
            this.loadingModal.show = false;
          })
          .finally(() => {
